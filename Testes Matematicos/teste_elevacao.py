...
def main():
    ...
    antenna_params = []
    sensitivity = []
    for hingeMultip in np.arange(LOWSIZE,HIGHSIZE+((HIGHSIZE-LOWSIZE)/HINGESTEPS),(HIGHSIZE-LOWSIZE)/HINGESTEPS):
        for multiplier in np.arange(LOWSIZE,HIGHSIZE+((HIGHSIZE-LOWSIZE)/HINGESTEPS),(HIGHSIZE-LOWSIZE)/HINGESTEPS):
            print(f'Calculando para pole {multiplier:.4f} e hinge {hingeMultip:.4f}',end='                                       \r')
            pole = ref_pole.copy()
            antenna_hinge = ref_antenna_hinge.copy()
            antenna_pole = ref_antenna_pole.copy()
            multiplied_pole = tuple(item * multiplier for item in ref_pole[0])
            multiplied_hinge = tuple(item * hingeMultip for item in ref_antenna_hinge[0])
            pole[0] = multiplied_pole
            antenna_hinge[0] = multiplied_hinge
    ...
    iteracao = 1
    while latch:
        print(f'Iteração {iteracao}',end='\r')
        iteracao += 1
        if reset_params:
            reset_params = False
            best_guess_value = (-360,360)
        ...
        for i in range(len(sensitivity)):
            ...
            # Checks if has higher range
            if abs(actuator[0] - actuator[1]) > abs(best_guess_actuator[0] - best_guess_actuator[1]):
                reset_params = True
                best_guess_actuator = actuator
            # Checks if covers rotation
            elif (abs(value[1] - value[0]) >= ANTENNA_ANGLE_TOLERANCE+MAX_ANTENNA_ANGLE-MIN_ANTENNA_ANGLE) and (abs(actuator[0] - actuator[1]) == abs(best_guess_actuator[0] - best_guess_actuator[1])):
                #Check if has lower overhead
                if abs(value[1] - value[0]) <= abs(best_guess_value[1] - best_guess_value[0]):
                    print('\nMelhor parâmetro encontrado!')
                    print('Range:',abs(actuator[0] - actuator[1]))
                    print('Delta:',abs(value[1] - value[0]))
                    print(best_guess_actuator, "<=", actuator)
                    print(best_guess_value, "<=", value)
                    print(best_guess_index, "<=", i)
    ...
    for correc_angle in np.arange(-180,180,ANGLE_CORRECTION_STEP):
        ...
        if min_val < MIN_ANTENNA_ANGLE and max_val > MAX_ANTENNA_ANGLE:
            if abs(min_val-MIN_ANTENNA_ANGLE)+abs(max_val-MAX_ANTENNA_ANGLE) < best_guess_difference:
                ...
                best_guess_angles = (min_val,max_val)
    # Printar parâmetros:
    print('--- Melhores parâmetros ---')
    best_guess_params = antenna_params[best_guess_index]
    print(f'pole = {calcular_tamanho(best_guess_params[0]):.4f}')
    print(f'antenna_hinge = {calcular_tamanho(best_guess_params[1]):.4f}')
    print(f'antenna_pole = {calcular_tamanho(best_guess_params[2]):.4f}')
    print(f'antenna_correction = {best_guess_correction:.4f}')
    print(f'actuator_range = {best_guess_actuator}')
    print(f'antenna_range = {best_guess_angles}')
    ...
    antenna_pole = pole[0]
    result_array = np.add(pole[0], (0,0.1)) 
    result_tuple = [pole[0],tuple(result_array)]
    antenna_pole = result_tuple
    numcolor = 21
    for actuator_len in np.arange(calcular_tamanho(ref_actuator)*LOWBORDER,(calcular_tamanho(ref_actuator)*HIGHBORDER),calcular_tamanho(ref_actuator)/STEPS):
        active_actuator = [(0,0),(actuator_len,0)]
        try:
        ...
        new_active_act_pos = calcular_posicao(active_actuator,(90*DEG2RAD)-actuator_angle)
        new_act_pos = calcular_posicao(ref_actuator,(90*DEG2RAD)-actuator_angle)
        new_antenna_pole = calcular_posicao(antenna_pole,(90*DEG2RAD)+hinge_angle-(best_guess_correction*DEG2RAD))
        # Calcular sensibilidade do sistema
        # Ângulo atual da antena
        actual_angle = calcular_angulo(new_antenna_pole)/DEG2RAD
        # Ver se ângulo maior é maior que o ângulo esperado
